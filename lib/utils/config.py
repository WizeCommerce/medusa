...
class Config():
    ...
    class __impl:
        ...
        def config_dir(self):
            """
            return the location of the config directory.  Default is config
            """
            try:
                return os.path.join(self.__repo_dir__, self.__config_dir__)
            except:
                return None
    ...
    def __init__(self, config=None):
        __config_dir__ = os.path.join(os.getcwd(), "config")
        if config is None:
            config = config = os.path.join(__config_dir__, "appConfig.yaml")
        if Config.instance is None:
            # Create and remember instance
            Config.instance = Config.__impl(config)
