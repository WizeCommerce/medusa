...
from lib.utils.naming_helper import cap_convert
import subprocess
import re
...
class RubyClient(Client):
    ...
    def __sshDeployArtifact__(self, file):
        ...
        """
        self.log(file)
        host = self.config.get_ruby_option(key="ssh_host")
        user = self.config.get_ruby_option(key="ssh_user")
        path = self.config.get_ruby_option(key="ssh_path")
        cmd = ("scp %s %s@%s:%s" % (file, user, host, path))
        self.log("executing:  " + cmd)
        exit_code = os.system(cmd)
        if exit_code != 0:
            self.log("failed to execute: {cmd}".format(cmd=cmd))
            sys.exit(exit_code)
        self.log("succeded at executing:  " + cmd)
        #install gem
        ## -t -t setups a fake tty mode
        cmd = "ssh -t -t %s@%s 'sudo gem install %s'" % (user, host, os.path.join(path, os.path.basename(file)))
        self.log("executing:  " + cmd)
        ...
        #Cleanup
        cmd = "ssh  %s@%s 'rm -v %s/*.gem'" % (user, host, path)
        os.system(cmd)
        return 0
    ...
    def __build_dependency__(self, thriftFile):
        """
            called by base class and builds all dependencies and deploy the artifact
            either locally or globally based on env. settings.
        """
        self.thrift_helper.check_file(thriftFile)

        properties = self.thrift_helper.read_thrift_properties(thriftFile)
        object_dependencies = self.thrift_helper.read_thrift_dependencies(thriftFile)
        ...
        properties["GEM"] = self.__generateInjectionCode__(thriftFile)
        ##fix artifactID
        properties = self.fix_properties(properties)


        #build Gem file
        return self.deploy_object(properties, thriftFile)
