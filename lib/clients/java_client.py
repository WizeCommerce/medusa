...
class JavaClient(Client):
    ...
    def __deploy_production_artifact__(self, properties, thrift_object, postfix=""):
        ...
        check_value_shaded = self.check_shaded_version(groupId=properties.get("GROUPID"), artifactId=properties.get("ARTIFACTID"),
                                         version=properties.get("VERSION"))
        ...
        if (check_value and version_check) or snapshot_check:
        ...
        if ((check_value_shaded and version_check) or snapshot_check) and (self.config.is_uber and self.thrift_helper.is_service(thrift_object)):
              #copy uber jar
              ...
              exit_code = subprocess.call(shlex.split(self.config.maven_deploy_command))
              if exit_code is not 0:
                  print "WARN:  failed to publish shaded jar's pom.xml file"
    ...
    def check_shaded_version(self, **kwargs):
        ...
        url = self.config.get_java_option("java_host_all") + properties['groupId'] + "/" + properties['artifactId'] + \
              ...
              self.generate_artifact_name(groupId=properties['groupId'], artifactId=properties['artifactId'] + postfix, version=properties['version'])
    ...
    def check_version(self, **kwargs):
        ...
        url = self.config.get_java_option("java_host") + properties['groupId'] + "/" + properties['artifactId'] + "/" + \
              str(properties['version']) + "/" + \
              self.generate_artifact_name(groupId=properties['groupId'], artifactId=properties['artifactId'], version=properties['version'])
